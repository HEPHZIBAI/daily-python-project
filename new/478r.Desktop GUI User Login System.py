'''
Project Description
    Today’s project is to build a real-world login system with a graphical user interface (GUI) using Python. 
    This app will allow users to register and log in with a username and password, and after a successful login, they’ll be taken to a new screen that acts like a personal dashboard.
    The login system will store user credentials in a local file using the JSON format. 
    This means that the app will remember registered users even after you close it. 
    It's a great way to simulate how basic authentication works in real-world desktop applications — no database or internet connection required.
...
The GUI will be built using Tkinter, Python’s standard library for building desktop apps. 
You’ll learn how to handle button clicks, keyboard events, pop-up messages, and file I/O — all in one project.
